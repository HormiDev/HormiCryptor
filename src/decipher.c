...
int main (int argc, char *argv[]) {
	...
	out_file_name = malloc(strlen(argv[1]) - 4);
	...
	strncpy(out_file_name, argv[1], strlen(argv[1]) - 4);
	if (out_file_name == NULL)
	{
		write(2, "Error: cannot allocate memory\n", 30);
		close (in_file);
	...
	while (read_bytes > 0)
	{
		vigenere_decipher(buffer, key, read_bytes);
	...
	close(in_file);
	close(out_file);
	printf("File %s has been decrypted\n", argv[1]);
	unlink(argv[1]);
	return 0;
}